...
/// <summary>
/// FluentValidation validator for CreateAssociationDto
/// </summary>
public class CreateAssociationDtoValidator : AbstractValidator<CreateAssociationDto>
{
    private readonly IAssociationService _associationService;

    public CreateAssociationDtoValidator(IAssociationService associationService)
    {
    ...
    private async Task<bool> BeUniqueAssociationNumber(string associationNumber, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(associationNumber))
            return true;

        return await _associationService.IsAssociationNumberUniqueAsync(associationNumber, null, cancellationToken);
    }

    private async Task<bool> BeUniqueClientCode(string clientCode, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(clientCode))
            return true;

        return await _associationService.IsClientCodeUniqueAsync(clientCode, null, cancellationToken);
    }
